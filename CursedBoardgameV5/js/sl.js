...
{	
	...
	if(tfcounter==0)
	{
		prog = "She somehow made it through entirely unscathed. Perhaps fate has other plans...";
	}
	else if(tfcounter<=3)
	{
		prog = "She made it through nearly unchanged. Well, except for a few things...";
	}
	else if(tfcounter<=9)
	{
		prog = "She'll have a couple of changes to get used to...";		
	}
	else
	{
		prog = "She has changed so much she's barely recognizable. Luck was not on her side.";
	}
	
	
	document.getElementById('prognosis').innerHTML = prog;
	document.getElementById('tfcount').innerHTML = "Total Curses Received: " + tfcounter + " | Cash Earned: $" + money + "/month";
}
...
var nextscreen = function(oldscreen, newscreen) {
	...
	{
	populateboard();
	showpopup("<img src='img/welcome.jpg' width=200 height=125 style=\"border:2px solid #000000; float:middle;\" /><br/>Welcome " + playertoken.stats["name"] + ". Someone went to great lengths to punish you for being such a judgy prude.<br/><br/>Your punishment will be left to fate, and determined by this game. Roll the dice to reach the end of the game and escape. Each roll will give you choices, but there's technology at work here that may as well be magic, and those choices will change you. If you're unlucky... you'll leave as nothing more than a hole to be fucked.<br/><br/>We've brought this stranger to witness to your punishment, and to act as a neutral 3rd party to ensure you finish.<br/>");
	}
}
...
var setQuestion = function(fixeddata)
{
	...
	if(currcelldata)
	{
		...
		else if(currcelldata.category=="CursedTransformation")
		{
			...
			var qtyglyph = qtyitem("Passcode: Protection");
			if(qtyglyph)
			{
				currentOptions.push({category: "CursedTransformation", effectname: "Activate Passcode: Protection",  subtitle: (qtyglyph+" Remaining"), img:"none", value: 0});
			}
		}
		...
		else if(currcelldata.category=="CursedTeleport")
		{
			...
			if(qtyglyph)
			{
				currentOptions.push({category: "WildMagic", effectname: "Activate Passcode: Protection",  subtitle: (qtyglyph+" Remaining"), img:"none", value: 0});
			}
		}
		...
		{			
			...
			var qtyglyph = qtyitem("Passcode: Unlocking");
			if(qtyglyph)
			{
				currentOptions.push({category: "TreasureChest", effectname: "Activate Passcode: Unlocking",  subtitle:(qtyglyph+" Remaining"), img:"none", value: 0});
			}
		}
		...
		{				
			...
			if(mycoins>=3)
			{
				currentOptions.push({category: "ItemShop", effectname: "Passcode: Protection", subtitle:"Costs 3 Silver Coins", img:"none", value: 3});
			}
			if(mycoins>=2)
			{
				currentOptions.push({category: "ItemShop", effectname: "Passcode: Jaunting", subtitle:"Costs 2 Silver Coins", img:"none", value: 2});
			}
			if(mycoins>=1)
			{
				currentOptions.push({category: "ItemShop", effectname: "Passcode: Unlocking", subtitle:"Costs 1 Silver Coin", img:"none", value: 1});
			}
...
function ApplyEffect(seldat)
{
	...
	{		
		...
		switch(seldat.effectname)
		{
			...
			case "Passcode: Jaunting":
			case "Passcode: Protection":
			case "Passcode: Unlocking":
				showpopupafter = false;
				giveitem(seldat.effectname,1);
				useconsumable("Silver Coin",seldat.value);
				showpopup("<u>Purchased Item</u><br/><br/><span style='color:yellow'><em>\"Thank you for your purchase!\"</em></span><br/><br/><span style='float:right'>The terminal deducts your coins and credits you the selected passcode.<br/><br/><span style='color:cyan'>Purchased "+seldat.effectname+" for "+seldat.value+" silver coins.<br/><br/>Item added to inventory.</span></span>","shop2big.jpg");
				
			break;			
			
			case "Activate Passcode: Protection":
				showpopupafter = false;
				showpopup("<u>Activate Passcode: Protection</u><br/><br/><span style='color:yellow'><em>\"Not Today.\"</em></span><br/><br/><span style='float:right'>You activate a Protection Passcode, negating whatever was in store for you.<br/><br/><span style='color:cyan'>Used 1 Passcode: Protection</span></span>","dispel.jpg");
				useconsumable("Passcode: Protection",1);
			break;
			
			case "Activate Passcode: Unlocking":
			case "Open Chest":
			{
				showpopupafter = false;
				...
				if(seldat.effectname=="Activate Passcode: Unlocking")
				{
					useconsumable("Passcode: Unlocking",1);
					isTrapped = false;
				}
				...
				{									
					...
					showpopup("<u>Trapped Chest</u><br/><br/><span style='color:yellow'><em>\"You chose...poorly.\"</em></span><br/><br/><span style='float:right'>The chest was trapped! The terminal emits an ominous noise and she feels herself transforming!<br/><br/><span style='color:cyan'>Gained Random Curse: "+trap.effectname+"</span></span>",
				...
				else
				{
					...
					showpopup("<u>Treasure Looted</u><br/><br/><span style='color:yellow'><em>\"Congratulations, you've earned a reward for your bravery!\"</em></span><br/><br/><span style='float:right'>You open the trasure chest and obtain a reward! (automatically added into inventory)<br/><br/><span style='color:cyan'>Got Item: "+randitem.name+" x "+giveqty+"</span></span>","goodchest.jpg");
			...
			case "Hair Growth": 
			PrepareTF();
			attribute="hair length"; adjust_attrib(attribute,1);	
			break;

			case "Enfeeblement": 
			PrepareTF();
			attribute="strength"; adjust_attrib(attribute,-10); 
			break;

			case "Breast Growth": 
			PrepareTF();
			attribute="breast size"; adjust_attrib(attribute,1); 
			break;
...
//			break;
			...
			case "Ass Expansion": PrepareTF();attribute="ass size"; adjust_attrib(attribute,1); 
			break;
			...
			case "Increased Libido": PrepareTF();attribute="increased libido"; adjust_attrib(attribute,1); 
			break;

			case "Increased Sensitivity": PrepareTF();attribute="increased sensitivity"; adjust_attrib(attribute,1); 
			break;

			case "Increased Fluids": PrepareTF();attribute="increased fluids"; adjust_attrib(attribute,1); 
			break;

			case "Always Ready": PrepareTF();attribute="always ready"; adjust_attrib(attribute,1); 
			break;

			case "Enhanced Orgasms": PrepareTF();attribute="enhanced orgasms"; adjust_attrib(attribute,1); 
			break;

			case "Submissiveness": PrepareTF();attribute="submissiveness"; adjust_attrib(attribute,1); 
			break;
			
			case "Hypnotic Susceptibility": PrepareTF();attribute="hypnotic susceptibility"; adjust_attrib(attribute,1); 
			break;

			case "Multiple Orgasms": PrepareTF();attribute="multiple orgasms"; adjust_attrib(attribute,1); 
			break;

			case "Random Orgasms": PrepareTF();attribute="random orgasms"; adjust_attrib(attribute,1); 
			break;

			case "Triggered Orgasms": PrepareTF();attribute="triggered orgasms"; adjust_attrib(attribute,1);
			break;

			case "Triggered Arousal": PrepareTF();attribute="triggered arousal"; adjust_attrib(attribute,1);
			break;

			case "Easily Aroused": PrepareTF();attribute="easily aroused"; adjust_attrib(attribute,1); 
			break;

			case "Hair Trigger": PrepareTF();attribute="hair trigger"; adjust_attrib(attribute,1); 
			break;

			case "Flexible": PrepareTF();attribute="flexible"; adjust_attrib(attribute,1); 
			break;

			case "Cheerful": PrepareTF();attribute="cheerful"; adjust_attrib(attribute,1); 
			break;

			case "Tasty Fluids": PrepareTF();attribute="tasty fluids"; adjust_attrib(attribute,1); 
			break;

			case "Pheromones": PrepareTF();attribute="pheromones"; adjust_attrib(attribute,1); 
			break;

			case "No Gag Reflex": PrepareTF();attribute="no gag reflex"; adjust_attrib(attribute,1); 
			break;

			case "Oral Lover": PrepareTF();attribute="oral lover"; adjust_attrib(attribute,1); 
			break;

			case "Anal Lover": PrepareTF();attribute="anal lover"; adjust_attrib(attribute,1); 
			break;

			case "Infertile": PrepareTF();attribute="infertile"; adjust_attrib("very fertile",-1);adjust_attrib(attribute,1); 
			break;

			case "Very Fertile": PrepareTF();attribute="very fertile"; adjust_attrib("infertile",-1); adjust_attrib(attribute,1); 
			break;

			case "Pent Up": PrepareTF();attribute="pent up"; adjust_attrib(attribute,1); 
			break;

			case "Masochistic": PrepareTF();attribute="masochistic"; adjust_attrib(attribute,1); 
			break;

			case "Exhibitionist": PrepareTF();attribute="exhibitionist"; adjust_attrib(attribute,1); 
			break;

			case "Lewd Dreams": PrepareTF();attribute="lewd dreams"; adjust_attrib(attribute,1); 
			break;
			...
			{
				playertoken.stats["height"] -= 1;
				playertoken.stats["height"] = playertoken.stats["height"]<minheight?minheight:playertoken.stats["height"];
			}
			break;

			case "Heat": PrepareTF();attribute="heat"; adjust_attrib(attribute,1); 
			break;

			case "Wallflower": PrepareTF();attribute="charisma"; adjust_attrib(attribute,-10); 
			break;

			case "Dependent": PrepareTF();attribute="dependent"; adjust_attrib(attribute,1); 
			break;

			case "Fluid Addiction": PrepareTF();attribute="fluid addiction"; adjust_attrib(attribute,1); 
			break;

			case "Lactation": PrepareTF();attribute="lactation"; adjust_attrib(attribute,1); 
			break;

			case "Incontinence": PrepareTF();attribute="incontinence"; adjust_attrib(attribute,1); 
			break;

			case "Hair Removal": PrepareTF();attribute="hair removal"; adjust_attrib(attribute,1); 
			break;

			case "Sleepy": PrepareTF();attribute="sleepy"; adjust_attrib(attribute,1); 
			break;

			case "Ditzy": PrepareTF();attribute="ditzy"; adjust_attrib(attribute,1); 
			break;

			case "Noisy": PrepareTF();attribute="noisy"; adjust_attrib(attribute,1); 
			break;

			case "Orgasm Denial": PrepareTF();attribute="denial"; adjust_attrib(attribute,1); 
			break;

			case "Clumsy": PrepareTF();attribute="dexterity"; adjust_attrib(attribute,-10); 
			break;

			case "Enervation": PrepareTF();attribute="stamina"; adjust_attrib(attribute,-10); 
			break;

			case "Glasses": PrepareTF();attribute="eyesight"; adjust_attrib(attribute,-10); 
			break;

			case "Bad Luck": PrepareTF();attribute="luck"; adjust_attrib(attribute,-10); 
			break;

			case "Lust Crest": PrepareTF();attribute="lust crest"; adjust_attrib(attribute,1); 
			break;

			case "Mutual Orgasms": PrepareTF();attribute="mutual orgasms"; adjust_attrib(attribute,1); 
			break;
			
			case "Potent Partner": PrepareTF();attribute="potent partner"; adjust_attrib(attribute,1); 
			break;

			case "Breeding Fetish": PrepareTF();attribute="breeding fetish"; adjust_attrib(attribute,1); 
			break;

			case "Palette Swap": 
			PrepareTF();
			attribute=""; adjust_attrib("palette swap",1);
			var hrc = ["Black","Blonde","Brown","Platinum BLonde","Gray","White","Pink","Red","Auburn"];
			...
			extrastr += "<br/><br/>Her hair changes to a "+toLowerFirst(playertoken.stats["hair color"])+ " color, and her eyes shift to a shade of "+toLowerFirst(playertoken.stats["eye color"])+"!";
			break;

			case "Name Change": 
			PrepareTF();
			attribute=""; adjust_attrib("name change",1);
			var femname = ["Jenny","Brandi","Megan","Kylie","Sammy","Lexi","Alexis","Danielle","Candy","Emily","Chloe"];
			playertoken.stats["name"] = femname[Math.floor(Math.random() * femname.length)];
			extrastr += "<br/><br/>Her name is now "+playertoken.stats["name"]+ " - you both know this instinctively. It feels strange to think of her by any other name.";
			break;
...
function selectsandbox(ele)
{
	...
	document.getElementById('bio').innerHTML = "First Name: <input type=text id=ifname value='Sarah' maxlength=15 size=8><br/>"
	+"Last Name: <input type=text id=ilname value='Smith' maxlength=15 size=8><br/>"
	+"Age: <input type=number id=iage value=24 min=18 max=80><br/>"
	+"Height (cm): <input type=number id=iheight value=180 min="+minheight+" max=200><br/>"
	+"Gender: <select id=igender><option value=1>F</option></select><br/>"
...
function stringify_player(verbose)
{
	...
	if(verbose)
	{
		...
		arr.push("denial");
		arr.push("lust crest");
		arr.push("mutual orgasms");
		arr.push("potent partner");
		arr.push("breeding fetish");
...
function stringify_attrib_desc(attribute, includeLabel ,describeAllChange)
{
	...
	if(fx) //if there's a readymade description, use it
	{
		...
		{
			ret = "Her "+attribute+" is unremarkably average.";
		}		
		if(ret!="" && includeLabel)
		{
	...
	{
		case "height":
			overrideDesc = "Her height is now " + toFeet(playertoken.stats["height"])+" (" + playertoken.stats["height"] +"cm)";
		break;
		case "breast size":
		if(describeAllChange && ret!="")
		{
			overrideDesc =  "Her "+attribute+" is now " + ret;
		}
		break;		
		case "hair length":
		case "hair color":
		if(describeAllChange && ret!="")
		{
			overrideDesc =  "Her hair is now " + toLowerFirst(ret) + ".";
		}
		break;
		case "ass size":
		if(describeAllChange && ret!="")
		{
			overrideDesc =  "Her ass is now " + toLowerFirst(ret);
		}
		break;
	}
